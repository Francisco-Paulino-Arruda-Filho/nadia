...
class ForAllIntroductionDef(Rule):
    def validate(self, context: RuleContext) -> bool:
        if len(context.references) != 1:
            context.errors.append("ForAll Introduction requires 1 reference")
            return False
        ref = context.references[0]
        f = context.symbol_table.lookup_formula_by_line(context.line, ref.value)
        if not isinstance(context.formula, QuantifierFormula) or not context.formula.is_forall():
            context.errors.append("Formula must be universal quantifier")
            return False
        return True
    ...
    def get_latex_notation(self, context: RuleContext) -> str:
        ...
        return f"\\infer[\\!\\!{{\\forall i}}]{{{lhs}}}{{{latex_r}}}"

    def get_name(self) -> str:
        return "ForAll Introduction"
